...
class Brand_reports{
   ...
   /* get customers details for dashboard display */
    public function cusDashDetails($connection){
        ...
        $sql="SELECT concat(u.First_Name,' ',u.Last_Name)AS Name,concat(u.Postalcode,' , ',u.Street,' , ' ,u.City)As Address, c.Contact_No, p.Quantity,po.Category,o.Amount,o.Delivery_Method,o.Order_date,pay.Paid,o.Delivery_Status FROM `order`o INNER JOIN shop_placeorder p on o.Order_id=p.Order_Id INNER JOIN user u ON u.User_id=p.Customer_Id INNER JOIN user_contact c ON u.User_id=c.User_id  INNER JOIN product po ON po.Item_code=p.Product_id INNER JOIN payment pay ON o.Order_id=pay.Order_Id WHERE (o.Order_Status=1 && p.StockManager_id=$this->User_id && pay.User_Id=$this->User_id) ORDER BY o.Time ASC LIMIT 5";
        ...
        if($result->num_rows===0){
            ...
            while($row=$result->fetch_assoc()){
                ...
                array_push($answer,['Name'=>$row['Name'],'Address'=>$row['Address'],'Contact_No'=>$row['Contact_No'],'Quantity'=>$row['Quantity'],'Category'=>$row['Category'],'Amount'=>$row['Amount'],'Order_date'=>$row['Order_date'],'Delivery_Method'=>$row['Delivery_Method'],'Paid'=>$row['Paid'],'Delivery_Status'=>$row['Delivery_Status']]);
    ...
    public function DeliveredOrderDetails($connection){
        $this->User_id=$_SESSION['User_id'];
        $sql="SELECT concat(u.First_Name,' ',u.Last_Name)AS Name,concat(u.Postalcode,' , ',u.Street,' , ' ,u.City)As Address, c.Contact_No, p.Quantity,po.Category,o.Amount,o.Delivery_Method,o.Order_date,pay.Paid,o.Delivery_Status FROM `order`o INNER JOIN shop_placeorder p on o.Order_id=p.Order_Id INNER JOIN user u ON u.User_id=p.Customer_Id INNER JOIN user_contact c ON u.User_id=c.User_id  INNER JOIN product po ON po.Item_code=p.Product_id INNER JOIN payment pay ON o.Order_id=pay.Order_Id WHERE (o.Order_Status=1 && p.StockManager_id=$this->User_id && pay.User_Id=$this->User_id && o.Delivery_Method='By delivery person' ) ORDER BY o.Time ASC";
    ...
    public function PickedOrderDetails($connection){
        $this->User_id=$_SESSION['User_id'];
        $sql="SELECT o.Order_id,concat(u.First_Name,' ',u.Last_Name)AS Name,concat(u.Postalcode,' , ',u.Street,' , ' ,u.City)As Address, c.Contact_No, p.Quantity,po.Category,o.Amount,o.Delivery_Method,o.Order_date,pay.Paid,o.Delivery_Status FROM `order`o INNER JOIN shop_placeorder p on o.Order_id=p.Order_Id INNER JOIN user u ON u.User_id=p.Customer_Id INNER JOIN user_contact c ON u.User_id=c.User_id  INNER JOIN product po ON po.Item_code=p.Product_id INNER JOIN payment pay ON o.Order_id=pay.Order_Id WHERE (o.Order_Status=1 && p.StockManager_id=$this->User_id && pay.User_Id=$this->User_id && o.Delivery_Method='Delivered by agent' ) ORDER BY o.Time ASC";
        ...
        if($result->num_rows===0){
            ...
            $answer=[];
            while($row=$result->fetch_assoc()){
                array_push($answer,['Order_id'=>$row['Order_id'],'Name'=>$row['Name'],'Address'=>$row['Address'],'Contact_No'=>$row['Contact_No'],'Quantity'=>$row['Quantity'],'Category'=>$row['Category'],'Amount'=>$row['Amount'],'Order_date'=>$row['Order_date'],'Delivery_Method'=>$row['Delivery_Method'],'Paid'=>$row['Paid'],'Delivery_Status'=>$row['Delivery_Status']]);
            }
            return $answer;
        }
