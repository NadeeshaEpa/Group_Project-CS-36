...
class delivery_model
{
    ...
    {      
        $sql="SELECT o.Order_id,o.Order_date,o.Delivery_Status,u.First_Name,u.Last_Name,o.DeliveryPerson_Id,i.imgname from `order` o INNER JOIN `user` u ON o.DeliveryPerson_Id=u.User_id INNER JOIN `profileimg` i ON u.User_id=i.User_id WHERE o.Delivery_Method='Delivered by agent' AND o.Order_Status='1' AND o.Delivery_Status='1' OR o.Delivery_Status='0' ORDER BY o.Order_id DESC";
    ...
    {      
        $sql="SELECT o.Order_id,o.Order_date,o.Time,u.First_Name,u.Last_Name,p.Customer_Id,i.imgname from `order` o INNER JOIN `placeorder` p ON o.Order_id=p.Order_id INNER JOIN `user` u ON P.Customer_Id=u.User_id INNER JOIN `profileimg` i ON u.User_id=i.User_id WHERE o.Delivery_Method='Delivered by agent' AND o.DeliveryPerson_Id IS NULL AND o.Order_Status='1' UNION SELECT o.Order_id,o.Order_date,o.Time,u.First_Name,u.Last_Name,p.Customer_Id,i.imgname from `order` o INNER JOIN `shop_placeorder` p ON o.Order_id=p.Order_id INNER JOIN `user` u ON P.Customer_Id=u.User_id INNER JOIN `profileimg` i ON u.User_id=i.User_id WHERE o.Delivery_Method='Delivered by agent' AND o.DeliveryPerson_Id IS NULL";
        $result = mysqli_query($connection, $sql);
        $delivery = [];
        if ($result) {    
            while ($row = mysqli_fetch_assoc($result)) {
